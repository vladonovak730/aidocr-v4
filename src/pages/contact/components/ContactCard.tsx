...
export const ContactCard: FC<IContactCardProps> = ({image, title, contactInfos}) => {
  return (
    <div className="p-[14px] bg-white flex flex-col items-center w-full justify-center">
      ...
      <ul className="flex flex-col items-center gap-[10px]">
        {contactInfos.map((item) => (
          <li key={item.value} className={clsx("5xl:text-[17px] text-[15px]", item.linkable && "hover:underline cursor-pointer")}>
            {item.linkable ? <a property="email" target="_blank" href={`${item.option}:${item.value}`}>info@aidocr.com</a> : item.value}
          </li>
        ))}
      </ul>
    </div>
